...
<template>
  <div class="card-standard rounded-lg">
    <div class="card-body p-4">
      <h3 class="card-title text-sm text-base-content/70">{{ title }}</h3>

      <div class="grid grid-cols-2 gap-4 mt-2">
        <div>
          <div class="stat-value-primary">{{ formatMinutes(props.minutes) }}</div>
          <div class="text-xs text-base-content/50 mt-0.5">Time Wasted</div>
        </div>

        <div>
          <div class="stat-value-critical">${{ formatCost(props.cost) }}</div>
          <div class="text-xs text-base-content/50 mt-0.5">Damages</div>
